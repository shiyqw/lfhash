...
template <typename T>
BenchmarkOptCuckooHashMap<T>::BenchmarkOptCuckooHashMap(
...
template <typename T>
void BenchmarkOptCuckooHashMap<T>::benchmark_read_only() {
...
template <typename T>
void BenchmarkOptCuckooHashMap<T>::benchmark_write_only() {
...
template <typename T>
void BenchmarkOptCuckooHashMap<T>::benchmark_read_only_single_bucket() {
	    ...
	    for (int i = 0; i < 10; i++) {
		    ...
		    for (int j = 0; j < num_readers; j++) {
		        pthread_create(&workers[j], NULL,
		        	thread_send_requests<OptimisticCuckooHashMap<T>>, &args[j]);
		    }
		    for (int j = 0; j < num_readers; j++) {
...
template <typename T>
void BenchmarkOptCuckooHashMap<T>::benchmark_space_efficiency() {
	...
	for (float space_efficiency = 0.15f; space_efficiency <= 0.9f; space_efficiency += 0.15f) {
	  int num_buckets = (1.0f/space_efficiency) * m_num_ops / float(m_slots_per_bucket);
	    OptimisticCuckooHashMap<T> my_map(num_buckets);
		double best_time = m_benchmark_reads_helper(&my_map);
...
template <typename T>
double BenchmarkOptCuckooHashMap<T>::m_benchmark_reads_helper(
		OptimisticCuckooHashMap<T>* my_map) {
...
template <typename T>
void BenchmarkOptCuckooHashMap<T>::run_all() {
